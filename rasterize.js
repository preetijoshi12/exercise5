...
function loadTriangles() {
    ...
    if (inputTriangles != String.null) { 
        ...
        for (var whichSet=0; whichSet<inputTriangles.length; whichSet++) {
            ...
            // fetch the diffuse color for this triangle set
            var diffuse = inputTriangles[whichSet].material.diffuse;
            
            // set up the vertex coord array and color array
            for (whichSetVert=0; whichSetVert<inputTriangles[whichSet].vertices.length; whichSetVert++) {
                vtxToAdd = inputTriangles[whichSet].vertices[whichSetVert];
                coordArray.push(vtxToAdd[0],vtxToAdd[1],vtxToAdd[2]);
                
                // push diffuse RGB for each vertex
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            gl_FragColor = vec4(vColor, 1.0); // use material diffuse color
        }
    `;
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                vertexPositionAttrib = gl.getAttribLocation(shaderProgram, "vertexPosition"); // pointer to vertex position
                gl.enableVertexAttribArray(vertexPositionAttrib);

                vertexColorAttrib = gl.getAttribLocation(shaderProgram, "vertexColor"); // pointer to vertex color
